...
To **jeden skill z plikami pomocniczymi**. Wywołujesz tylko `reklama-z-referencji`; agent sam sięga po instrukcje promptowania, analizy i kontroli jakości, gdy są potrzebne. Pozostałe pliki nie są osobnymi skillami i nie wymagają osobnych komend.
...
> Zainstaluj skill z https://github.com/alekszach/reklama-z-referencji/tree/main/skills/reklama-z-referencji

Agent z obsługą instalacji skillów pobierze cały katalog wraz z plikami pomocniczymi. To jeden skill; nie instaluj każdego pliku osobno. Jeśli używasz innego środowiska, pobierz repozytorium i skopiuj folder `skills/reklama-z-referencji` do katalogu skillów tego środowiska.

### Pierwsza reklama

> Użyj $reklama-z-referencji. Produkt: [link lub załączone zdjęcie]. Bohaterka: kobieta około 30 lat. Reklama po polsku. Najpierw stwórz postać, potem prześlę film referencyjny.

Jeśli masz już komplet:

> Użyj $reklama-z-referencji. Oto produkt, wybrana postać i film referencyjny. Przygotuj reklamę 30 sekund w podobnym rytmie, z własnym tekstem. Na razie pokaż scenariusz i prompty.
...
W pakiecie znajduje się [przykład po polsku](skills/reklama-z-referencji/references/przyklad.md), kontrola techniczna planu oraz [raport przeprowadzonych prób](SPRAWDZENIE.md). Pliki kampanii, prywatne media i dane konta trzymaj poza tym pakietem.
